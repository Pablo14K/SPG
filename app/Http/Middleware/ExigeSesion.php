...
class ExigeSesion
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        //
        // El plazo vive en `sgp.sesion.inactividad_min` y `SESSION_LIFETIME` va
        // más alto a propósito, para que el archivo de sesión siga estando
        // cuando este control tiene que hablar.
        $limite = (int) config('sgp.sesion.inactividad_min', 30) * 60;
        $ultima = (int) $request->session()->get('sgp_ultima_actividad', 0);

        if ($ultima > 0 && (time() - $ultima) > $limite) {
            Sesion::cerrar();
            flash('Cerramos tu sesión: pasaron más de '
                . (int) config('sgp.sesion.inactividad_min', 30)
        ...
        if (! $request->routeIs('portal.atencion_json')) {
            $request->session()->put('sgp_ultima_actividad', time());
        }
