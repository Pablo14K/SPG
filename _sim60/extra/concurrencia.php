...
// =========================================================================
$prod = DB::selectOne('SELECT p.id_producto, fn_producto_stock(p.id_producto) AS stock
                         FROM producto p WHERE p.activo=1 AND fn_producto_stock(p.id_producto) BETWEEN 1 AND 40
                        ORDER BY RAND() LIMIT 1');
if ($prod) {
    ...
    $sal = enParalelo($trabajos);
    $fin = (float) DB::scalar('SELECT fn_producto_stock(?)', [$idp]);
