...
> **Se llama SGP —Sistema de Gestión para Peluquería— desde la 7.116.0.** Hasta ahí se
> escribía «SPG» en todos lados, con las letras cambiadas, y el usuario lo corrigió junto con
> el subdominio: `sgp.columbiatcc.online`. El cambio es de nombre en TODO —pantallas, CSS
> (`.sgp-*`), JS (`SGPCarga`), comandos (`sgp:diagnostico`), `config/sgp.php`, la cabecera
> `X-SGP-Correo` en los dos proyectos, contenedores (`sgp_app`, `sgp_bd`…), imágenes, el
> router de Traefik y los documentos— **salvo tres cosas que quedan a propósito con la
> grafía vieja:**
>
> | Queda `spg` | Por qué |
> |---|---|
> | `/docker/spg`, la carpeta del panel de Hostinger | es el nombre que el panel le dio al proyecto en su día; describe el servidor, no el sistema |
> | `github.com/Pablo14K/SPG.git` | es el nombre del repositorio en GitHub; renombrarlo es del usuario, y GitHub redirige el viejo |
> | `spg_migracion`, en el historial | era el nombre de una tabla que ya no existe: cambiarlo sería reescribir lo que pasó |
>
> **El proyecto de Compose del servidor se llama `sgp` desde el 11/09/2026**, y con él los
> volúmenes: `sgp_datos_bd`, `sgp_almacenamiento`, `sgp_imagenes_*`. La 7.116.0 lo había
> dejado en `spg` a propósito —de ese nombre salen los volúmenes, y con otro se crean
> vacíos—, pero ese mismo día el usuario dio de baja el proyecto del servidor y lo volvió a
> armar como `sgp`: los volúmenes `spg_*` **ya no existen** y la operación se recuperó del
> respaldo de `/var/respaldos/spg/`. Desde entonces el `-p` correcto es **`-p sgp`**, y
> **volver a `-p spg` sería repetir el accidente**: levantaría con volúmenes nuevos y vacíos.
> Las fotos —de perfil, de servicios y el logo— no viajan en el `.sql` y se volvieron a
> subir a mano. Ver *Las fotos que sube el salón*.
>
> **Y el nombre del contenedor también cambió**, así que en el servidor hay dos cosas que
> apuntaban al viejo: el guion de respaldo copiado a `/usr/local/bin` y la línea del cron que
> lo llama —se vuelven a copiar después de desplegar, ver `ACTUALIZAR.md`—. La clave
> `sgp_bio` de `localStorage` también cambió: el panel de la huella deja de ofrecer la
> cuenta recordada hasta el próximo ingreso, una sola vez.
...
`config/sgp.php` (`sgp.version`) y se muestra en el pie de todas las pantallas.
...
### Al entregar una tanda, decí SI TOCÓ LA BASE y con qué se actualiza

**Toda tanda termina diciendo dos cosas: qué se tocó y qué hay que correr en el
servidor.** No alcanza con listar los cambios — quien despliega necesita saber
si le basta con subir el código o si además tiene que aplicar un guion sobre la
base, y son dos procedimientos distintos con dos riesgos distintos.

La pregunta se contesta de forma mecánica: **si el commit tocó
`basededatos/peluqueria_bd(base).sql`, tocó la base.** La regla del proyecto
obliga a regenerar ese archivo en la misma tanda que cualquier cambio de tabla,
columna, `CHECK`, vista, función, procedimiento o disparador, así que mirar si
cambió alcanza:
...
**Caso A — sólo código** (la mayoría de las correcciones). Nada que aplicar a
mano: el código viaja dentro de la imagen.
...
**Caso B — código y base.** Lo de arriba **no alcanza**: el importador del
arranque sólo actúa con la base vacía, a propósito, porque sobre una cargada
importar sería borrar la operación del salón. Van tres pasos más, y el respaldo
va **primero**:
...
| Qué se dice al entregar | Por qué |
|---|---|
| **«Sólo código»** o **«código y base»** | son dos procedimientos y dos riesgos: el segundo no se puede deshacer sin el respaldo |
| El **nombre del guion** de `basededatos/actualizaciones/` | sin eso hay que ir a buscarlo, y el comando queda apuntando a un archivo que no existe |
| Que **`down -v` nunca va en el servidor** | en desarrollo es lo normal y ahí borra el año de operación del salón |

> **Y si tocó la base, el guion tiene que existir.** Los cambios de esquema
> viajan como archivo versionado en `basededatos/actualizaciones/`, con la fecha
> y la versión en el nombre, re-ejecutable y sin tocar datos. Decir «tocó la
> base» sin dejar el guion es dejar el despliegue a medias.
...
| **cualquier cosa, antes de dar algo por terminado** | `docker compose exec app php artisan sgp:diagnostico` | es lo único que compara lo que corre contra lo que se entrega |
...
| La cuenta de correo cargada en **Seguridad → Correo del sistema** | ya no viaja en ningún archivo (7.105.0): sin ella el sistema llega **sin correo**, y una clienta nueva no puede terminar de registrarse. `sgp:diagnostico` y el panel lo dicen, pero conviene dejarla puesta antes de comprimir |
...
| 7. `sgp:diagnostico` y abrir el portal | que la aplicación arranque contra la base recién importada |
...
> hay que decirlas todas, y `sgp:diagnostico --produccion` está justamente para
...
> indistinguible de una rota: `sgp:diagnostico` tiene una sección de correo justamente porque
...
Qué es «terminado»: las pruebas en verde (`php artisan test`), `sgp:diagnostico` sin
...
app/
  ...
  Console/Commands/        sgp:diagnostico · sgp:pendientes · sgp:preparar-sql
                           sgp:notificaciones
config/
  sgp.php                  Versión, puntos, agenda, timbrado
...
routes/
  ...
  console.php              El scheduler: sgp:notificaciones cada diez minutos
public/assets/             app.css · imprimir.css · app.js · webauthn.js
                           imprimir.css estiliza `.sgp-imprimir`: el informe y los listados
...
rápidos (`.sgp-chip`) y las altas rápidas dentro de un formulario (`.btn-rapido`).
...
| Secundario | `.sgp-chip`, `.btn-rapido` | tres estados, ver abajo | atajos de navegación y altas rápidas |
| Terciario | `.sgp-rol-chip`, `.e-warn` | solo contorno, fondo transparente | información con jerarquía, no acción |
...
- **Para TEXTO sobre el fondo van `--carbon` y `--oro-enfasis`, nunca `--negro`
  ni `--oro-oscuro`.** Los dos primeros se invierten con el tema; los otros dos
  no sirven para eso — `--negro` es una **superficie** (en oscuro, el plano más
  profundo) y `--oro-oscuro` es el *pressed* de un botón dorado, pensado para
  fondos claros, y **no se invierte en absoluto**. Usarlos como color de letra
  deja el renglón ilegible: así quedaron el nombre de la clienta en la tarjeta
  del celular (negro sobre negro) y dieciséis lugares en oro oscuro a 2,3:1,
  hasta la 7.120.0. **Texto negro sobre un relleno dorado sí es `--negro`**, y
  tiene que seguir siéndolo: la regla es sobre el fondo de la página, no sobre
  un botón. Lo vigila
...
Por eso `sgp:diagnostico` tiene la sección **«Las fotos del salón»**: compara
...
- **Cinco niveles de navegación, y cada uno responde una pregunta distinta.** Si se saca
  ...
  |---|---|---|
  | Barra de módulos (`.sgp-nav`) | fija bajo el encabezado | *¿a qué otro módulo voy?* — el actual va marcado en oro |
  | Desplegable del módulo (`.sgp-nav-menu`) | al pasar el mouse por la barra | *¿a qué pantalla de ese módulo voy, sin pasar por la tarjeta?* |
  | Submenú lateral (`.sgp-nav-sub`) | al pasar el mouse por un grupo | *¿cuál de las de ese grupo?* — sólo donde hay grupos, hoy Tesorería. **La tarjeta del módulo no agrupa** (7.119.0, pedido del usuario): ocho tarjetas corridas como en los demás; `<x-landing>` conserva `grupo` por si hace falta |
  ...
  > **Había un cuarto nivel, los accesos rápidos (`.sgp-chip`), y salió en la 7.32.0 por
  ...
  > a `visible`** ahí, porque `.sgp-nav-in` scrollea en horizontal y un desplegable dentro de
...
> **Las pantallas de detalle se marcan con un cuarto valor en `false`** en
  ...
  > (7.57.0) —sus entradas son `seguridad.personal.index` y `seguridad.configuracion.index`—,
  > así que `configuracion.index` no existía y la miga salía «Panel › Datos de pago», sin el
  > módulo en el medio, en las ocho pantallas de esos dos módulos. Es el defecto del nombre de
  > ruta por cuarta vez (7.58.0, 7.62.0, 7.68.0), y como siempre sin dar error: la miga se
  > dibujaba, sólo que más corta. Corregido en la 7.115.1.
  >
  > **Y el sexto valor en `true` dice «sólo el Administrador».** Es para la pantalla que no
  > tiene submódulo propio porque no se puede conceder desde Roles —Correo del sistema, que
  > guarda el middleware `admin`—: su permiso declarado es el **módulo padre**
  > (`configuracion`), que dice dónde vive —la miga, el activo de la barra—, y el sexto valor
  > dice quién la ve. Sin esa marca `pantallasDe()` se la ofrecería a cualquiera con algo de
  > Configuración y le contestaría 403. `AndamiajeTest` lo fija en las dos direcciones, y
  > además comprueba que **toda tarjeta del landing esté en la barra**: es la otra mitad del
  > desfase que ya vigilaba —una pantalla que sólo viva en la tarjeta pasaba en verde—.
> **En pantalla angosta la barra de módulos es un CAJÓN**, no un riel. Se abre
  ...
  >
  > Va con una casilla escondida y su etiqueta (`#sgpCajon`), así que **se abre
  > con CSS y funciona con `app.js` caído**, igual que el desplegable. Y se
  > acota a `.sgp-nav-mod`: **la barra del portal no se convierte**, porque el
...
- **El pie va al fondo de la ventana, en todas las pantallas** (7.119.0): el
  ...
  columna y `main` estira; las pantallas de acceso traen su propio envoltorio.
...
| `.sgp-movil-titulo` | el dato principal de la tarjeta, sin rótulo |
| `.sgp-movil-sujeto` | **quién**: la clienta en la agenda, quién pagó en Cobros — en negrita y sin rótulo, que «CLIENTE» sobre el único nombre grande no aclara nada |
| `.sgp-movil-acciones` | la botonera al pie; los botones de sólo ícono con `.sgp-btn-ico` **escriben su `title`** al lado, porque en el celular no hay mouse que lo muestre |
...
- **Y lo que TRABA la cita va antes que «Detalle».** El aviso de que falta el
  fichaje —o de que a esa persona ya se la dio por ausente— iba después del
  botón, y se reportó al revés de como tiene que leerse: primero qué impide
  atender, que es lo accionable ahora, y después la ficha, que es información.
  Es esta misma regla aplicada al orden de la grilla de acciones.
...
- **En la agenda, «Detalle» abre una VENTANA y las acciones van en dos
  ...
  **fuera de la tabla**, por lo de la 7.87.4. Y los siete íconos seguidos
  ...
  > **Y el rótulo entra ENTERO** (7.120.0): iban en una línea con
  > `text-overflow:ellipsis`, así que en 133 px el más largo salía «Cambiar
  > profes…» —en la computadora y en el celular—. Un botón que no dice qué hace
  > es justo el problema que estas dos columnas vinieron a resolver, así que
  > antes que recortarlo se lo deja **envolver**: la grilla sube a 300 px, la
  > fila crece unos píxeles y los dos botones de cada renglón siguen midiendo lo
  > mismo porque la grilla estira.
  >
  > **Y chicos** (7.119.0): la primera versión los dibujaba del alto de un botón
  > normal —30 px, 143 de ancho— y con tres renglones la fila medía 116 px: «los
  > botones son demasiado grandes». Van del alto de un badge, 25 px, con la
  > grilla acotada a 266 px —lo justo para «Cambiar profesional» entero—, y la
  > fila vuelve a 95. **La fila es el resumen** —«Corte de dama ×2, Manicura»,
  > lo que dice `vw_agenda_citas`— y de quién es cada servicio se lee en la
  > ventana; y ahí **«Quién viene» es un renglón por persona**: quién es y qué
  > papel tiene, sus servicios, su alergia —«sin registrar» ES una respuesta— y
  > su ficha, o el botón de crearla. Eran tres listas que volvían a nombrar a
  > la misma gente.
...
El plazo vive en **`sgp.sesion.inactividad_min`** y lo hace cumplir
...
>
> **Y el CSS de `.logo-big` NO va scopeado bajo `.sgp-login`**, que es la
> segunda mitad del mismo error y costó la 7.120.0: la pantalla del enlace del
> correo usa este partial dentro de un `container` pelado, así que ninguna de
> las tres reglas le aplicaba y **el logo del salón salía a su tamaño natural**,
> una imagen de 900 px encima de la cita. Si agregás otra vista con la marca
> grande, no hace falta hacer nada; si alguna vez volvés a anidar esas reglas,
> `AndamiajeTest::la_marca_grande_no_depende_de_la_pantalla_de_ingreso` lo
> frena.
>
...
`.sgp-logo` y `.logo-big` son **pastillas doradas**, y ese oro es el fondo de la
...
| | Sin logo | Con logo |
|---|---|---|
| Clase | `sgp-logo` | `sgp-logo tiene-img` |
...
foto»**. Era el mismo defecto del logo, en el avatar: `.sgp-avatar` pinta el
...
| Qué | Dónde | Para qué |
|---|---|---|
| `sgp-avatar tiene-img` | el layout y Mi cuenta, cuando hay foto | apaga el fondo: mientras la foto llega no se ve *otro* avatar |
...
**Dónde está aplicado hoy** — 84 en 42 archivos:

| Qué se guardó | Cuántos |
|---|---|
| El subtítulo de `<x-encabezado>` | los 45, de una vez |
| Subtítulos escritos a mano (portal, panel, Mi cuenta) | 10 |
| Líneas de ayuda bajo un campo (`form-text`) | 48 |
| Párrafos explicativos bajo un título de sección | 22 |

**Y lo que NO se guardó, que es la mitad del criterio:**

- **Los subtítulos que muestran DATOS.** El teléfono y el correo en el historial
  de la clienta, la fecha y el número de la compra, quién abrió la caja y cuándo,
  el profesional y la hora de la atención en curso. Eso no explica la pantalla:
  **es** la pantalla.
...
**El sistema navega a la vieja usanza, así que cada pantalla es una foto.** En
un salón eso se nota: dos personas sobre la misma agenda, una registra la
atención y la otra la sigue viendo Programada hasta que se le ocurre recargar.
Lo mismo con la caja que abrió el otro mostrador, o con la cita que entra por el
portal mientras alguien mira el día.

| | |
|---|---|
| Cómo se enciende | `@section('vivo', 'agenda')` en la vista — **sin eso no consulta nada** |
...
- **Devuelve una huella y no datos.** Así no hay nada que filtrar por permiso
  más allá del acceso a la sección, y la respuesta pesa lo mismo con una cita
  que con doscientas. Qué cambió lo contesta recargar, que es lo que el
  navegador ya sabe hacer.
- **La huella no sale de un `updated_at`.** Ninguna de esas tablas lo tiene, y
  ...
  estados** cubren lo que de verdad pasa —entró una cita, se atendió, se
  canceló, se abrió un cajón— y se resuelven con un índice.
- **No recarga encima de algo escrito.** `hayAlgoQuePerder()` mira si hay un
  modal abierto o un campo tocado (`[data-sgp-tocado]`); si lo hay, **aparece un
  aviso con «Actualizar»** y la página no se mueve. Recargar sobre un formulario
  a medias es la peor forma de «tiempo real», y es la queja que este proyecto ya
  arregló dos veces con el borrador de las altas rápidas.
- **No consulta con la pestaña en segundo plano** (`document.hidden`), igual que
  el refresco del portal: no tiene sentido gastar datos por una pantalla que
  nadie está mirando. **Y por eso no cuenta como actividad de sesión**: si
  contara, una pestaña abierta y olvidada mantendría la sesión viva para siempre.

> **No abre un websocket, y es a propósito.** Una consulta cada veinte segundos
> sobre un `COUNT` indexado le cuesta menos al servidor que mantener la conexión
> abierta, y no agrega ninguna pieza más que instalar y vigilar.

> **Ojo al probarlo con el panel del navegador escondido**: ahí
> `document.hidden` es `true` y el navegador estrangula los `setInterval`, así
> que parece que no funciona. Hay que forzar la propiedad y despachar
> `visibilitychange` a mano.
...
|---|---|---|
| `.sgp-barra-carga` | la página entera se está yendo a buscar | barra de 3 px arriba de todo, en oro |
| `.btn.cargando` | ese botón disparó algo y está esperando | el ícono se vuelve spinner, sin cambiar el ancho |
| `.sgp-spinner` | un pedazo de pantalla se llena por `fetch` | spinner suelto, con `.sgp-cargando-texto` al lado |

Se maneja con **`SGPCarga`** (`app.js`), y las pantallas con JS propio lo usan en vez de
inventar el suyo:

```js
SGPCarga.envolver(fetch(url), bloqueQueSeVaARehacer)   // barra + atenúa el bloque
SGPCarga.ocupar(boton) / SGPCarga.liberar(boton)
```
...
- **Es un adorno, y tiene que poder faltar.** Las vistas con JS propio declaran
  `var SGPCarga = window.SGPCarga || { envolver: function (p) { return p; } };`, así que si
...
- **Los `select` de un filtro ofrecen LO QUE HAY para filtrar, no el catálogo.** Facturas →
  Comprobante ofrecía los ocho tipos de `tipo_comprobante` —seis dados de baja en la 7.9.0 y
  la 7.85.0, sin un solo comprobante— y elegir cualquiera devolvía la lista vacía: *«da
  opciones que no hay»*. Las opciones salen ahora de las **filas que la pantalla lista**
  (`Listado::opcionesUsadas($sql, $par)`, que recibe un SQL de dos columnas `k` y `v`), así
  que es dinámico —el día que se emita una nota de crédito, aparece sola— y no hace falta
  mirar `activo`, que tampoco alcanza: un tipo activo sin comprobantes sigue siendo una opción
  vacía, y uno dado de baja con comprobantes viejos sigue haciendo falta para encontrarlos.
  **El SQL va acotado IGUAL que la lista** —misma sucursal, mismas uniones—, o una opción
  devolvería cero filas, que es justo lo que se quiso evitar; por eso recibe el SQL entero y
  no un nombre de tabla. Vale para Facturas (Estado y Comprobante), Cobros (Medio de pago y
  Estado) e Inventario → Movimientos (Producto y Tipo, que ofrecía «Venta de producto», fuera
  de alcance desde la 7.23.1). Lo fija
  `ReglasDeNegocioTest::los_filtros_de_las_listas_ofrecen_solo_lo_que_hay`, que crea un tipo
  y un medio nuevos sin usar y exige que no se ofrezcan, y que lo ofrecido sea exactamente lo
  que hay.
...
`php artisan sgp:diagnostico` compara los dos relojes y avisa si no coinciden.
...
> `sgp:diagnostico`.** Ese número se quedó atrás **dos veces**: en 54 cuando la 7.0.0 lo llevó
...
al elegirlos. Lo arma `app.js` clonando el `<template class="sgp-cobro-molde">` de la vista, y
...
> **Las clases `sgp-cobro-*` no son decorativas: son las que busca el JS.** Si se renombran, el
...
| Pasado `sgp.agenda.sena_horas` | `Notificaciones::cancelarSenasVencidas()` la suelta **y le avisa** |
...
### Hay roles que NO llevan turno, y lo dice el rol

**`rol.exige_turno`** (por defecto 1) declara si las cuentas con ese rol
necesitan turno asignado. Sin marcar, el sistema deja de pedirlo y no las cuenta
como pendientes; se administra en **Seguridad → Roles**.

| | |
|---|---|
| Dónde se decide | `rol.exige_turno` — una casilla por rol |
| Quién lo hace cumplir | `Pendientes::profesionales()` y el bloque de turnos de la ficha |
| La excepción | **basta con que UNO de sus roles lo exija** |

> **El turno existe para que la agenda sepa cuándo atiende esa persona**, así que
> a quien no atiende no le hace falta ninguno: recepción, compras, caja. El aviso
> les pedía todos los días resolver algo que no era un problema, y **un aviso que
> no aplica enseña a ignorar los que sí**.
>
> **La 7.107.0 lo tapó a medias exceptuando al Administrador por id**
> (`permisos.rol_admin`), que es justo lo que este proyecto no hace en ninguna
> otra parte: los roles se crean desde una pantalla, así que un salón que creara
> «Recepción» volvía a tener el aviso sin forma de callarlo.

> **La excepción del rol de más se conserva y es la parte delicada.** Desde que
> una cuenta lleva varios (`usuario_rol`, el cambio de perspectiva), la dueña que
> además atiende tiene el rol Profesional encima: ahí el turno vuelve a hacer
> falta, porque la agenda sí la va a ofrecer. En la ficha, el bloque de turnos se
> muestra o se esconde leyendo `data-exige-turno` de cada casilla de rol — y
> **esconder no borra**: lo que ya estaba cargado se sigue guardando.
...
> leerlas. Es el único dato de la ficha que **puede lastimar a alguien** si se
> pasa por alto, así que tiene su propio lugar y su propio destaque.

> **Se ve en la fila, no escondida detrás del botón de detalle.** Todo lo demás
> de esa celda se guarda ahí porque se mira una vez al preparar el turno; una
> alergia hay que verla sin abrir nada. Es la excepción que la regla de la ayuda
> contextual ya declara: **lo que explica se guarda, lo que advierte no**.
...
### La agenda se mira por día, y también por tramo

El filtro **«Ver»** decide qué se está mirando: `''` (sólo ese día, lo que se
abre por defecto), `sem`, `mes`, `prox` y `todas`.

> **Mostraba UN día y no había forma de ver el conjunto.** Para contestar «¿qué
> tengo esta semana?» o «¿cuándo vino Carmen la última vez?» había que ir día por
> día con la flecha, y una cita de hace tres meses era inalcanzable en la
> práctica.

- **El día sigue siendo la pantalla de trabajo**, así que no cambia de forma: se
  abre sin filtro y conserva su orden **por lo que falta hacer** —lo que ocupa la
  agenda arriba, lo atendido después, lo que ya no va a ocurrir al final—.
- **En un rango manda la fecha.** Ese orden por estado mezclaría marzo con agosto
  en el mismo bloque; «Todas» va al revés, de lo más nuevo a lo más viejo.
- **Todo pagina**, y el día con página grande (100) para no partir en dos una
  jornada. «Todas» sobre un año de operación son miles de filas con seis
  subconsultas cada una.
- **Los marcadores son sólo los que la consulta usa.** La conexión prepara de
  forma nativa: pasarle uno de más contesta *Invalid parameter number* y la
  agenda entera devuelve 500. El día usa `:d1`; la semana y el mes, los dos.
...
| Cuándo | Qué hace `sgp:notificaciones` |
|---|---|
...
#### Y el mismo servicio para VARIAS personas de la cita

Dos amigas que vienen a cortarse el pelo marcan las dos en «Corte» (7.119.0),
y eso son **dos cortes**: dos filas de `cita_servicio`, una por persona
(`persona` 1 y 2), que la misma profesional hace una después de la otra. Hasta
ahí el único `uq_cita_servicio (id_cita, id_servicio)` rechazaba la segunda y
había que reservar dos citas; ahora es **`(id_cita, id_servicio, persona)`**.
Lo que sigue sin poder pasar es el mismo servicio dos veces **para la misma
persona**, que es lo que el único siempre quiso decir.

| Pieza | Qué hace |
|---|---|
| `Acompanantes::personaDe()` | lee `para[id_servicio][]` y devuelve `[id_servicio => [personas]]`; lo que falte es de la titular |
| `Agenda::vecesPorServicio()` | fija, **por petición**, cuántas veces va cada servicio; lo leen `turnos()`, `duracion()` y `bloques()` |
| `Agenda::ordenDeCopias()` | el turno de cada copia, para que `agendar()` guarde el suyo en cada fila |
| `veces[id]` en la consulta de horarios | el calendario mide los dos cortes, no uno; `app.js` lo manda con lo marcado |

- **La base no necesitó ninguna rutina nueva**: ya estaba escrita sobre filas.
  `fn_cita_duracion` suma lo de la misma profesional, `sp_emitir_factura` arma
  un renglón por fila y `trg_citaserv_bi` sólo mira OTRAS citas. Lo que cambió
  es el motor de PHP, que va por id de servicio y un id no puede aparecer dos
  veces en un arreglo — por eso la cantidad viaja aparte.
- **Las copias van con la MISMA profesional y en serie.** Es la limitación
  honesta del motor: repartir dos cortes entre dos peluqueras a la vez
  necesitaría un reparto por copia. Con dos personas y dos zonas distintas sí
  van a la vez, como siempre.
- **La atención registra una fila de `servicio_realizado` por persona**: con
  una sola, la segunda cabeza se trabajaba gratis para quien la hizo —
  `fn_comision_servicio` cuenta por fila.
- **Y una lista con ids repetidos también cuenta doble** —`duracion()` y
  `vecesPorServicio([], $lista)`—, que es como salen los servicios de
  `cita_servicio` en la reprogramación y en el enlace del correo.

> **El guion de actualización tiene el orden escrito**: `fk_cs_cita` se apoya
> en el único viejo —es el índice que empieza por `id_cita`—, así que el nuevo
> se crea ANTES de soltarlo. Y `vw_agenda_citas.servicios` dice «Corte ×2» en
> vez de «Corte, Corte». Lo fija
> `ReglasDeNegocioTest::el_mismo_servicio_puede_ser_para_varias_personas_de_la_cita`.
...
Los pasos son **Personas → Servicios → Profesionales → Fecha y hora → Detalles →
Confirmar** en el portal, y los mismos con **Cliente** adelante en Nueva cita.

> **«Personas» pregunta en el orden en que se contesta** (pedido del usuario,
> 7.119.0: «las preguntas de la primera sección están mal organizadas»). Iba
> cuántas → quiénes vienen → «no es para mí» → mis alergias: se preguntaba
> cuántas antes de saber quién se sienta en el sillón. Ahora **quién se
> atiende** —«Para mí» o «Para otra persona», dos radios con el mismo `name`
> (`para_otra_persona` = 0/1) y no una casilla escondida, con las alergias de
> ESA persona: «mis alergias» se esconde cuando la cita es para otra— y después
> **si viene alguien más** —cuántas en total y quiénes—. Las mismas tres en
> Nueva cita. Sin `app.js` se ven todos los bloques y se reserva igual.
...
- **Sin `app.js` se ven todos los pasos y se reserva igual.** El CSS esconde
  sólo bajo `.sgp-wiz-on`, y esa clase la pone el script.
...
> `sgp:asistente-paso` y los dos handlers escuchan ahí. El de profesionales sólo
...
- **Con avisos toma el oro del SISTEMA (`--oro`), el mismo del logo y del
  nombre del salón que tiene al lado.** Estaba en `--oro-claro`, que es el oro
  del *hover*: dos dorados distintos a dos centímetros uno del otro. El hover
  sigue siendo el claro, como todos los enlaces de la barra.
- **Y el numerito no se come la campana.** Con el ícono en 17 px la pastilla le
  tapaba media cara y el conjunto se leía como una mancha rosa. El texto va en
  `--blanco`, que se invierte: en claro es blanco sobre el rojo oscuro (7,9:1) y
  en oscuro es oscuro sobre el rojo claro (7,3:1) — con el `#fff` fijo que tenía,
  en oscuro quedaba blanco sobre rosa, en 2,2:1.
...
> `sgp:diagnostico` muestra el enlace **armado de verdad**, que es la única forma de ver de
...
- **Es del `admin` del middleware, no un submódulo.** Un submódulo se puede conceder a otro
  ...
  se agrega a mano en la landing con `Permisos::esAdmin()`, **y en el catálogo de navegación
  va con el sexto valor en `true`**, que es lo que la pone en el desplegable de la barra sólo
  para el Administrador. Hasta la 7.115.1 no estaba en el catálogo —«la card sí, el menú
  no»— y se reportó como *«Correo del sistema no está en la barra de navegación»*: era la
  única pantalla del sistema que vivía en una tarjeta y no en la barra.
...
- **Vacío es que NO sale ningún correo**, y desde la 7.105.0 eso es la regla y no un
  ...
  **No queda en silencio**, que es la condición: `sgp:diagnostico` lo cuenta como problema, el
...
- El despacho lo hace el comando **`php artisan sgp:notificaciones`**, que el scheduler corre
...
> `<x-cobro-lineas>`, `<x-ayuda>`, `<x-servicio-tarjeta>` y `<x-ciudad>`.
> Quedaba además su CSS (`.sgp-aviso-stock`, `.sgp-aviso-item`, `.sgp-aviso-mas`) sin marcado
...
## Sus comprobantes: la clienta los ve y los baja

| | |
|---|---|
| Dónde | **Portal → Mis citas**, en la columna «Comprobante» de cada cita anterior |
| Ver | `portal.factura_ver` — la pantalla, para mirar cuánto salió |
| Bajar | `portal.factura_descargar` — el PDF, para presentarlo en algún lado |

> **El endpoint de descarga existía desde la 7.42.0 y no se llegaba a él.** El
> único enlace vivía en la pantalla de la atención en curso, así que el
> comprobante se podía bajar **durante las dos horas de la cita y nunca más**:
> quien lo necesitaba para rendir un gasto lo pedía por WhatsApp.

Tres cosas al tocarlo:

- **La pantalla y el PDF comparten `portal/_factura_cuerpo`.** Escritos aparte
  se desfasan, y ahí la clienta termina con **dos documentos que dicen cosas
  distintas del mismo cobro** — el error que este proyecto ya se hizo con el
  desglose de la seña. Lo que cambia entre los dos es el envoltorio: el PDF lleva
  los colores escritos a mano porque **Dompdf no lee `app.css` ni las variables
  CSS**.
- **La pertenencia se comprueba en la consulta, no después.** El id viaja en la
  URL: `WHERE f.id_factura = ? AND f.id_cliente = ?`, con el cliente sacado de la
  sesión. La factura de otra clienta contesta **404**.
- **Una factura acreditada lo dice.** Con una nota de crédito vigente el
  comprobante ya no vale por lo que dice, y callarlo dejaría a la clienta
  presentando como gasto algo que el salón le devolvió.
...
(`.sgp-graf-pista` y `.sgp-graf-barra`), y traer Chart.js para eso agregaría una
...
- **El nivel** (Bronce → Platino) lo calcula `fn_cliente_nivel` por **cantidad de visitas**.
  Quién está en cuál no se asigna a mano; **desde cuántas visitas empieza cada uno
  y con qué descuento SÍ se administran**, en Servicios → Promociones.
  ...
  >
  > **Y dos niveles no pueden arrancar en el mismo número.** `fn_cliente_nivel`
  > elige por `visitas_minimas`, así que con dos en 10 la clienta cae en uno de
  > los dos según el orden interno de la tabla: el descuento que le toca pasaría
  > a depender de algo que nadie eligió.
- **Los puntos** los acumula la app al emitir el comprobante: `Facturacion::acumularPuntos()`
  ...
  **Ese número lo decide el salón, no el código**: vive en `configuracion.puntos_cada_gs` y se
  ...
  bajarlo es fijar cuánto regala el salón—. `config/sgp.php` conserva el valor como
...
**Desde la 7.0.0 el SGP se acopla al Automatizador SIFEN**, que es un proyecto aparte
...
**El SGP no habla con la DNIT ni firma nada.** Toma un comprobante que ya emitió y numeró
...
> El descuento del SGP vive por factura (`factura_descuento`), que es lo
...
>
> **El precio de lista SÍ viaja, como séptimo campo opcional del ITM.** Con los
> renglones ya netos, del otro lado no quedaba rastro de que hubiera existido un
> descuento: el KuDE imprimía los precios unitarios corridos (75.000 como 74.648)
> y sin nada que lo explicara.
>
> **Desde la 7.115.2 el Automatizador lo declara como lo modela el SIFEN, y el
> KuDE imprime lo mismo.** `InvoiceFactory` toma el campo 7 como precio unitario
> (E721 `dPUniProSer`) y la diferencia con el campo 5 como descuento particular
> por ítem (EA002 `dDescItem`, con su EA003 `dPorcDesIt`); EA008 `dTotOpeItem`
> sigue saliendo del neto, así que **el total del comprobante no cambia ni un
> guaraní**. En el pie del XML van F009 `dTotDesc` y F011 `dDescTotal` — iban
> sumados en F009 y F011 quedaba en cero, que sin descuentos no se notaba. **Y el
> KuDE lo muestra en su columna, en MONTO** (pedido del usuario): PRECIO
> UNITARIO, DESCUENTO, EXENTA, 5 %, 10 %, con las dos últimas centradas.
>
> | | Antes | Ahora |
> |---|---|---|
> | PRECIO UNITARIO | 100.000 | 100.000 |
> | descuento | %DESC **20** | DESCUENTO **20.000** |
> | IMPORTE 10% | 80.000 | 80.000 |
> | fila DESCUENTO al pie | «DESCUENTO: 0 %» | **no hay** |
>
> **La fila DESCUENTO del pie se fue**, por pedido del usuario: el descuento es
> uno solo y va desglosado por renglón. Iba en cero y se leía como que no hubo
> ninguno; la 7.108.0 la había puesto en el monto repartido y con eso el pie
> dejaba de sumar —159.250 − 750 ≠ 159.250—, y encima agregó una fila «IMPORTE
> SIN DESCUENTO» para explicar la diferencia. Los importes de la derecha son
> netos, así que SUBTOTAL y TOTAL cierran solos.
>
> **La forma en que se extendió el formato es la parte que importa:**
>
> | | Se declara | Un Automatizador viejo |
> |---|---|---|
> | Campo 5 · precio | **el neto**: de ahí sale EA008 y el total | lo usa igual: el total no cambia |
> | Campo 7 · precio de lista | E721 y, por diferencia, EA002 | **lo ignora** y declara el neto sin descuento: mismo total |
>
> Mandar la lista en el campo 5 y el descuento aparte sería lo «natural» y es
> justo lo que **no se puede hacer**: un Automatizador que ignorara ese campo
> declararía el total **sin descontar**, o sea de más ante la DNIT. El formato se
> extiende sólo por donde el error posible es cosmético.
>
> > **Ojo con `SifenXmlBuilder`**: usaba `ea008` —ya neto— como E727
> > `dTotBruOpeItem` y le volvía a restar el descuento para EA008. Con descuento
> > cero no se notaba; con descuento, el total del ítem salía descontado dos
> > veces. E727 es E721 × cantidad y EA008 es lo que la calculadora ya dejó.
>
> El campo va **sólo cuando hubo prorrateo**: sin descuento repetiría el mismo
> número y sería ruido en el archivo que se guarda como prueba de lo enviado.
...
- **Lo que NO se manda son los códigos geográficos del SIFEN**
  (departamento/distrito/ciudad). Son de una tabla oficial que el SGP no tiene,
...
#### La cita de varias: el comprobante de CADA una, de punta a punta

La base emite por persona desde la 7.117.0 (`sp_emitir_factura(…, p_persona)`), y hasta
la 7.119.0 **las pantallas no**: `emitir` no leía `persona` —el cobro individual mandaba
ahí con `?persona=2` y se emitía de toda la cita—, la pantalla del receptor tampoco la
arrastraba, y la lista excluía toda cita con alguna factura, así que la segunda amiga se
quedaba sin la suya. Se reportó tal cual: *«hace la factura por el pago grupal y deja una
deuda de lo que le corresponde a la otra persona, pero luego no se genera el comprobante»*.

| Dónde | Qué |
|---|---|
| Emitir, la fila | **¿de quién?** — «Toda la cita» o cada persona con lo suyo y su parte; la que ya tiene el suyo se dice con su número |
| `citaFacturable($idCita, $idTipo, $persona)` | «ya tiene factura» son dos preguntas: una de TODAS cierra la cita; las de algunas dejan emitir sólo las que faltan, y apagan «toda la cita» |
| Receptor | recibe `persona`, lista sólo sus renglones y dice de quién es |
| La lista de emitir y «falta facturar» | la cita a medio facturar **sigue** hasta que cada una tenga el suyo |
| Cobros | **«Falta cobrar N atenciones»** arriba del historial; el botón abre la ventana de cobro en la agenda (`?cobrar=<cita>`) o Facturas si lo que se debe está en un comprobante |

Lo fija `ReglasDeNegocioTest::la_cita_cobrada_por_persona_se_factura_por_persona_y_la_otra_despues`,
que recorre el camino real —el cobro desde la agenda, la pantalla de emitir con ella
elegida, la del receptor, y la segunda después— y con el `persona` ignorado falla donde
estaba el defecto.

> **Con comprobante, lo que falta cobrar lo dice el COMPROBANTE, no `fn_cita_total`**
> (7.122.1). `fn_cita_total` resuelve el descuento con las reglas vigentes **hoy**, y la
> factura lo congeló el día que se emitió: una promoción de un solo día hacía que al día
> siguiente la cita valiera el precio de lista y «Falta cobrar» reclamara una deuda que no
> existe, con un botón que llevaba a Facturas a no encontrar nada. La lista suma
> `fn_factura_saldo` de los comprobantes de venta vigentes (signo 1: una nota de crédito no
> es deuda) y mide contra la cita **sólo la parte sin comprobante**. La agenda ya lo hacía
> bien —muestra el saldo de la factura—; el que medía mal era Cobros. Lo fija
> `ReglasDeNegocioTest::cobros_no_reclama_lo_que_la_factura_ya_saldo_aunque_la_promo_haya_vencido`.
>
> **Regla general: `fn_cita_total` es para lo que TODAVÍA no se facturó.** Sobre una cita
> facturada es la cuenta de hoy, no la de ese día.

> **La ventana de cobro es ancha y en dos columnas en la computadora** (pedido del usuario):
> la cuenta a la izquierda —qué se cobra y por qué— y el pago a la derecha —cómo pagan, con
> qué, a qué caja—. `sgp-cobro-2col`, una columna en el celular.
...
- **Y sólo bajo los servicios reservados CON quien cierra** (pedido del
  usuario, 7.119.0). Listaba todos los de la cita —«los que el profesional
  puede hacer de los que hay en la cita»— y lo que usó la otra lo carga la
  otra, con su parte. El Administrador ve los grupos de todas, cada uno con
  de quién es (`data-prod-de`), y al elegir de quién cierra los demás se
  esconden.
...
> `sgp:pendientes` también lo reporta, que es donde tendría que verse primero.
...
La factura se emite en el SGP y **ya es válida**; declararla ante la DNIT es un paso posterior,
...
| El SGP apuntándole | `SIFEN_MODO=http` y `SIFEN_URL=http://sifen:8090/`, que es el **nombre del servicio** y se resuelve solo en la red de los contenedores, igual que `bd`. El `SIFEN_TOKEN` tiene que ser el `SIFEN_API_TOKEN` de *su* `.env`, o contesta 401 — y el SGP lee un 401 como RECHAZADO |
| El correo, **del lado del SGP** | la cuenta de «Seguridad → Correo del sistema». El Automatizador **no manda**: el SGP le dice que no en cada emisión —ver abajo—, y su `MAIL_*` queda vacío. Para probar el circuito de aquel lado sin mandar correo de verdad está `MAIL_TRANSPORT=file`, que escribe el `.eml` en `logs/emails/` |

**El Automatizador vive fuera del repositorio**, así que el compose lo monta por ruta: lo busca
como carpeta hermana del proyecto, y si está en otro lado se le dice con `SGP_SIFEN_PATH`. Dos
...
- **Si la carpeta no está, el contenedor avisa y se apaga solo.** Un servidor sirviendo una
  carpeta vacía contestaría **404**, y el SGP lee cualquier 4xx como **RECHAZADO** — o sea, «no
...
Comprobado así: el Automatizador toma el TXT que arma el SGP, genera el KuDE en PDF (78 KB, sin
librerías) y arma el correo con el **PDF y el XML adjuntos**. El SGP lo alcanza por HTTP desde
...
>
> El servidor arrastraba el `false` de la plantilla, y `sgp:diagnostico --produccion` terminaba
...
| `sifen_salida` | el XML firmado y el KuDE que genera. El SGP se baja su copia a `storage/app/sifen/<factura>/`, así que esto es el original |
...
El comprobante lo manda **uno solo**: el SGP, con la cuenta de «Seguridad →
...
|---|---|---|
| `X-SGP-Correo: no` | el SGP, en cada emisión | **el que manda**: no depende de nada del otro lado |
...
|---|---|
| **El SGP** | la de «Seguridad → Correo del sistema» — con el KuDE y el XML adjuntos, desde la copia local |
...
Ahora **lo decide el SGP en cada emisión**: manda `X-SGP-Correo: no`
...
> anterior a la 7.105.0 que no entiende la cabecera. El SGP lo dice en pantalla
...
Al declarar, el SGP **se baja el KuDE en PDF y el XML y los guarda** en
...
> Los dos proyectos siguen separados: **el SGP le habla sólo por HTTP** y no se copió código
...
las nueve acciones que **sacan o meten plata en el cajón de quien opera**: cobrar,
anular un cobro, anular un comprobante, la seña, el movimiento de efectivo, el pago
al personal y su reversión, y el pago a proveedor y su anulación. Antes solo lo pedía
el cobro, así que un pago a proveedor entraba con la caja cerrada y aparecía recién al
día siguiente, en el arqueo equivocado. El mensaje dice **qué se iba a hacer** («Abrí
...
> clienta sin su comprobante hasta el día siguiente, que es lo que la 7.42.0 corrigió.
> La nota **sí** descuenta el efectivo desde la 7.109.0, pero la caja de la que sale
> no es la de la sesión: es una del local **que emitió la factura**, así que la resuelve
> ella misma con `Caja::abiertasDe()` — ver *La devolución de una nota de crédito*.
...
### La devolución de una nota de crédito sale del cajón que se ELIGE

Emitir la nota **descuenta el efectivo del cajón en el mismo acto**, y de cuál
sale se pregunta: entre las cajas abiertas **del local que emitió la factura**,
no las del local donde está parada la persona.

| | |
|---|---|
| Cuánto sale | sólo lo que la clienta pagó **en efectivo**, prorrateado si la nota es parcial |
| De qué cajón | `Caja::abiertasDe()` sobre la sucursal de la factura — el mismo bloque `facturacion/_caja_elegir` que el cobro, la seña y los dos pagos |
| Con una sola abierta | no se pregunta, pero **se dice cuál es** |
| Con varias y ninguna elegida | **no se adivina**: la nota sale igual y la devolución queda pendiente |

> **Esto invierte a propósito lo que decidió la 7.48.0**, por pedido del usuario:
> *«Nota de crédito no disminuye caja (al disminuir debe dar como opción a qué
> caja disminuir —perteneciente a esa sucursal—)»*. Aquella versión sacó el
> egreso de la emisión porque se podía cargar **dos veces** —la emisión escribía
> uno y la clase «Devolución al cliente» dejaba cargar otro a mano, con montos
> distintos—; el segundo paso resolvía eso pero **casi nadie lo daba**, así que
> el cajón seguía diciendo que esa plata estaba adentro.
>
> **La duplicidad sigue estando cubierta, y no por un `if`**: el índice único
> `uq_movcaja_devolucion (id_factura, activo)` no admite una segunda salida
> vigente por la misma nota, y `notasPorDevolver()` descarta la que ya tiene su
> egreso — así que deja de ofrecerse en Movimiento de efectivo. Comprobado sobre
> la factura 001-001-0000062: Caja 1 pasó de Gs. 531.250 a **370.000**, Caja 2
> quedó intacta, y la nota desapareció del selector de devoluciones pendientes.

**«Movimiento de efectivo» sigue siendo el segundo camino, y hace falta**: es a
donde cae la devolución que no pudo salir del cajón. Los tres casos:

| Por qué no salió | Qué pasa |
|---|---|
| No se eligió caja habiendo varias abiertas | la nota se emite y la devolución queda pendiente |
| Ese local no tiene ninguna caja abierta | ídem, y la pantalla lo avisa **antes** de emitir |
| La caja elegida no tiene saldo suficiente | ídem, diciendo cuánto hay y cuánto hace falta |

> **La nota SIEMPRE se emite, y eso no es un descuido.** Es un comprobante fiscal
> con su número: cancelarlo porque el cajón está corto o porque falta abrirlo
> sería apagar una función que hoy anda —la regla del proyecto sobre eso está
> unas secciones más arriba— y **su número no se reutiliza**, así que tampoco se
> puede volver a emitir después. Lo que no puede pasar es que la devolución no
> ocurra **en silencio**: el aviso nombra el motivo y dice dónde quedó.

Cuatro cosas más que conviene no perder:

- **El monto sale del documento, no se tipea.** Es lo que impide que vuelvan a
  quedar dos números para la misma devolución.
- **El id del POST no se cree**: se comprueba que esa caja esté abierta **y** que
  sea del local de la factura. Si no, cambiando un número oculto se le saca plata
  al cajón de otra sucursal.
- **La sucursal sale de `factura.id_sucursal`**, con el timbrado de respaldo:
  `vw_factura_resumen` no la trae, y `Caja::abiertasDe(0)` cae en
  `Sucursales::activa()` — o sea que el combo ofrecía los cajones del local
  equivocado, que es justo lo que el guardado después rechaza.
- **El egreso va en su propio `try`.** Si falla, la nota queda emitida igual y
  vuelve a aparecer como devolución pendiente: perder un número de la SET por un
  problema de caja sería el peor de los dos males.

> **Ojo con el orden al tocar el índice único**: la clave foránea
> `fk_movcaja_factura` se apoya en él, así que hay que soltarla antes. Es la
> trampa que este documento ya anota en *Cambiar el esquema*, y apareció al
> comprobar la prueba en reversa.
...
> sistema arranca igual y `sgp:diagnostico` dice que está apagado, que es mejor que no
...
> **Y si igual se cuela, ahora se nota.** `sgp:diagnostico` distingue «el driver dice smtp»
...
`sgp:diagnostico` lo dice en la primera línea, y es lo primero que conviene correr cuando el
...
El sistema se publica en **`https://sgp.columbiatcc.online`**, sobre un VPS de Hostinger que
**viene con Docker**. El dominio se compró entre varios grupos de la facultad, así que el SGP
...
- **WebAuthn necesita HTTPS y el dominio como `rpId`.** No hay nada que configurar —lo toma de
  ...
  > con el subdominio y HTTPS, funciona. `SGPBio.estado()` lo explica en pantalla en vez de
  ...
  > autorización por huella dice `sgp.columbiatcc.online` en vez del nombre del
...
contraseña de la base queda descargable. `sgp:diagnostico --produccion` lo comprueba.
...
> sin `fn_promo_vigente` ni `fn_descuento_monto_factura`. `sgp:diagnostico` lo detecta.
...
**Las 230 pruebas corren contra `peluqueria_test`**, no contra una base de mentira: es la única
...
>
> Por eso `sgp:diagnostico` compara ahora las columnas de la base contra las del
...
**Casi todo lo que se rompió acá es la misma falla vista de nueve formas**: algo
...
Vale tenerlos nombrados, porque el próximo va a tener una de estas nueve formas:
...
| **Una pantalla anunciada en un lado y no en el otro** | sale en el menú y no en la tarjeta, o al revés | `AndamiajeTest::el_landing_de_cada_modulo_ofrece_todas_sus_pantallas`, **en las dos direcciones** desde la 7.115.1 |
...
> pregunta. `SGPConfirmar` lo reemplaza con un modal de Bootstrap, y **cae de
...
**230 pruebas** contra `peluqueria_test`. No prueban PHP: prueban que **las reglas de la base
...
- **Si `peluqueria_test` tiene otros datos —el respaldo del servidor, por ejemplo—, la batería
  se corre contra una copia del mes simulado**, que es la base con la que se escribieron sus
  premisas. `phpunit.xml` fija la base con `<env>` sin `force`, así que una variable del
  proceso le gana, y los procesos hijos de las pruebas de concurrencia la heredan:
  ```bash
  ...
  ```
  Sobre datos reales fallan las que dependen de la forma del mes simulado, y eso no dice nada
  del código: lo que hay que mirar ahí es si alguna falla por un **error** —un `TypeError`, una
  columna— y no por una premisa.
...
versión; se acopló al Automatizador, que es un proyecto aparte, sin copiar su código: el SGP
