...
> router de Traefik y los documentos— **salvo cuatro cosas que no se pueden renombrar sin
> perder algo, y quedan a propósito con la grafía vieja:**
>
> | Queda `spg` | Por qué |
> |---|---|
> | **`-p spg`**, el nombre del proyecto de Compose en el servidor | de él salen los nombres de los volúmenes —`spg_datos_bd`, `spg_almacenamiento`, `spg_imagenes_*`—: con otro nombre se crean **vacíos** y la base y las fotos del salón quedan huérfanas. Ver *Las fotos que sube el salón* |
...
>
> **Y el nombre del contenedor sí cambió**, así que en el servidor hay dos cosas que apuntaban
> al viejo: el guion de respaldo copiado a `/usr/local/bin` y la línea del cron que lo llama
> —se vuelven a copiar después de desplegar, ver `ACTUALIZAR.md`—. La clave `sgp_bio` de
> `localStorage` también cambió: el panel de la huella deja de ofrecer la cuenta recordada
> hasta el próximo ingreso, una sola vez.
