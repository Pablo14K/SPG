...
| | Qué |
|---|---|
| El Automatizador corriendo | `php -S 0.0.0.0:8090 -t .../sifen_automatizador/public`. Está en `.claude/launch.json` como `automatizador-sifen`. No necesita Composer: no tiene dependencias |
| El SPG apuntándole | `SIFEN_MODO=http` y `SIFEN_URL`. **Desde el contenedor es `host.docker.internal`, no `localhost`** — localhost adentro del contenedor es el contenedor. El `SIFEN_TOKEN` tiene que ser el `SIFEN_API_TOKEN` de *su* `.env`, o contesta 401 |
