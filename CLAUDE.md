...
```bash
docker exec spg_app sh -c 'mysql --skip-ssl -hbd -uroot -p"$DB_PASSWORD" --default-character-set=utf8mb4 peluqueria_bd < basededatos/actualizaciones/<el guion de esta versión>.sql'
```
...
- **No recarga encima de algo escrito.** `hayAlgoQuePerder()` mira si hay un
  modal abierto o un campo tocado (`[data-spg-tocado]`); si lo hay, **aparece un
