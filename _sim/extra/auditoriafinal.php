...
foreach (DB::select('SELECT id_producto, nombre, fn_producto_stock(id_producto) AS stock FROM producto') as $p) {
...
foreach (DB::select('SELECT id_caja, monto_inicial, id_estado_caja, fn_caja_saldo(id_caja) saldo FROM caja') as $c) {
    ...
    $mio = round((float) $c->monto_inicial + $ef + $ing - $egr - $pp, 2);
...
if ($pagoPers > 0 && (int) DB::scalar('SELECT COUNT(*) FROM movimiento_caja') === 0) {
    $mal('AUD_PAGO_PERSONAL_FUERA_DE_CAJA',
        'Se liquidaron ' . number_format($pagoPers, 0, ',', '.') . ' Gs. al personal y la caja no registró ni un egreso: '
        . 'fn_caja_saldo no resta pago_personal y no existe ninguna pantalla que escriba en movimiento_caja', 'ALTO');
}
...
if ($ventas === 0) {
    $mal('AUD_SIN_VENTA_DE_PRODUCTOS',
        'En 90 días no se facturó ni un producto: `detalle_factura.id_producto` no lo escribe ningún controlador. '
        . '`producto.precio_venta`, el tipo de movimiento 7 «Venta de producto» y el disparador trg_detfactura_ai '
        . 'están para un camino que no tiene pantalla', 'MEDIO');
}
