...
{
    if ($dow === 7) return 0;
    $base = [1 => 4, 2 => 6, 3 => 6, 4 => 7, 5 => 9, 6 => 11][$dow] ?? 5;
    if (in_array($dia, [40, 41, 42, 43], true)) $base += 5;      // temporada alta
    if (in_array($dia, [55, 56], true)) $base = 1;               // dos días flojos
    return max(0, $base + mt_rand(-2, 2));
}
...
function faseTarde(int $dia, int $dow): void
{
    ...
    if ($dia % 12 === 3) {
        ...
        $prods = DB::select('SELECT p.id_producto, p.nombre, p.precio_costo FROM producto p
                              WHERE p.activo=1 ORDER BY fn_producto_stock(p.id_producto) ASC LIMIT 5');
    ...
    if ($dia % 17 === 5) {
        $p = DB::selectOne('SELECT id_producto FROM producto WHERE activo=1 AND fn_producto_stock(id_producto) > 3 ORDER BY RAND() LIMIT 1');
        if ($p) {
...
function faseCierre(int $dia, int $dow): void
{
    ...
    if ($a->entrar('admin', 'admin123')) {
        if ($finDeMes) {
            foreach (array_merge(array_values(PROFS), ['karen']) as $u) {
                $idu = (int) (DB::scalar('SELECT id_usuario FROM usuario WHERE username=?', [$u]) ?: 0);
                if ($idu) {
                    $a->post('/facturacion/pagos/personal', ['id_usuario' => $idu, 'periodo' => date('m/Y')])->seguir();
                }
            }
