...
$s['stock_final'] = [];
foreach (DB::select('SELECT p.id_producto, p.nombre, p.unidad_medida, COALESCE(ps.stock_minimo,0) AS stock_minimo_v,
                            fn_producto_stock(p.id_producto, 1) AS stock,
                       ...
                       FROM producto p
                       LEFT JOIN producto_sucursal ps ON ps.id_producto = p.id_producto AND ps.id_sucursal = 1
                      ORDER BY p.id_producto') as $p) {
    $s['stock_final'][] = [
        'id' => (int) $p->id_producto, 'nombre' => (string) $p->nombre,
        'unidad' => (string) $p->unidad_medida, 'minimo' => (float) $p->stock_minimo_v,
