...
// E. Inventario
// =========================================================================
$p = DB::selectOne('SELECT id_producto, fn_producto_stock(id_producto, 1) AS stock FROM producto WHERE activo=1 ORDER BY RAND() LIMIT 1');
if ($p) {
    $idp = (int) $p->id_producto;
    $st0 = (float) DB::scalar('SELECT fn_producto_stock(?, 1)', [$idp]);
    foreach ([
        ...
        $a = (float) DB::scalar('SELECT fn_producto_stock(?, 1)', [$idp]);
        $rec->post('/inventario/cargar-stock', $d)->seguir();
        $b = (float) DB::scalar('SELECT fn_producto_stock(?, 1)', [$idp]);
        if (abs($b - $a) > 0.0001) {
    ...
    if ((float) DB::scalar('SELECT fn_producto_stock(?, 1)', [$idp]) < 0) {
