/**
 * Genera el informe de QA en Word a partir de _sim30/log/resumen.json.
 *
 *   NODE_PATH=<ruta a node_modules con docx> node _sim30/informe.js [salida.docx]
 */
...
const SALIDA = process.argv[2]
  || path.join(path.dirname(__dirname), 'Informe_QA_Simulacion_Intensiva_1_Mes_SGPLaravel.docx');
...
// deja el informe describiendo los 30 días y nada más.
...
H.push(ricos([
  'Se sometió al SGP ', ['versión 7.36.4', true], ' a una simulación de ',
...
if (top.length) {
  ...
  H.push(p('No se registró ningún incidente durante los 30 días.', { color: VERDE, bold: true }));
}
H.push(salto());
...
// =======================================================================
H.push(h1('5. Evolución de los 30 días'));
...
if (criticos.length === 0) {
  H.push(ricos([
    ...
    ' Ninguna de las categorías que comprometen el sistema —corrupción de datos, acceso no autorizado, error financiero, pérdida de información, duplicación grave o bloqueo de la operación principal— se materializó en los 30 días.',
  ]));
  ...
  H.push(tabla(['Riesgo', 'Cómo se forzó', 'Resultado'], [
    ...
    ['Bloqueo de la operación', { t: '30 días seguidos de agenda, atención, facturación, cobro y cierre', align: AlignmentType.LEFT }, { t: 'Sin interrupción', color: VERDE, bold: true }],
...
H.push(ricos([
  'La clasificación se apoya únicamente en la evidencia de estos 30 días. El sistema sostuvo ',
...
H.push(tabla(['Qué', 'Por qué'], [
  ...
  [{ t: 'libfaketime instalado en el contenedor', align: AlignmentType.LEFT }, { t: 'Mover el reloj de cada proceso para simular 30 días en orden cronológico.', align: AlignmentType.LEFT }],
...
[
  ...
  'python _sim60/analizar.py && python _sim60/graficos.py && node _sim30/informe.js',
...
const doc = new Document({
  ...
  title: 'Informe de Simulación Operativa y QA — SGP 7.36.4',
    ...
    footers: {
      default: new Footer({
        children: [new Paragraph({
          ...
          children: [
            new TextRun({ text: 'SGP 7.36.4 · Informe de Simulación Operativa y QA · ', size: 15, color: GRIS, font: 'Calibri' }),
